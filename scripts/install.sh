...
# recente do Prosa e instala com o gerenciador de pacotes da distro. Cobre
# openSUSE Leap e Fedora (mesmo RPM genérico do electron-builder, sem specs
# por distro como no Vega) e Ubuntu/Debian (.deb), ambos gerados por
# .github/workflows/release.yml. Em Arch use o pacote no AUR (`yay -S prosa`),
...
#
# PROSA_VERSION=v4.1.0 sudo -E bash install.sh   # trava numa tag específica
set -euo pipefail
...
EOF
  ...
  *)
    ...
    echo "" >&2
    echo "Alternativa universal: baixe o AppImage na página de releases e rode" >&2
    echo "  chmod +x Prosa-*.AppImage && ./Prosa-*.AppImage" >&2
    echo "(não precisa de root nem instalação):" >&2
    echo "  https://github.com/$REPO/releases" >&2
    exit 1
    ;;
esac
